...
for n_gbtree, n_gblinear in zip([[1,1],[5,5],[10,10]],[[1,1],[5,5],[10,10]]):
    ...
    for fold_num in [5]:
        ...
        j = "_lc4"
        start = time.time()
        index = list(range(5)) # num of folds
        train_file = output_file_list
        test_file = "./dataset/keck_extended/lc4/keck_lc4.csv"
        print train_file
        ...
        for my_final_model in my_final_model_list:
            ...
            y_test = np.array(np.repeat(0, len(y_pred_on_test))) # create fake label.
            y_test[1:50] = 1
            ...
            if store_prediction:
                ...
                test = pd.DataFrame({'label':np.nan,'test_pred':y_pred_on_test})
