...
class readData(object):
        ...
        '''
        def __init__(self,data_loc,label_name=None):
            """
            Parameters:
            -----------
            ...
            # Testing data does not have label_name 
        ...
        def read(self):
            ...
            if self.__label_name is not None:
        ...
        def label(self):
            ...
            if not isinstance(self.__y_data, np.ndarray):
               raise ValueError('You must call `read` before `label` and '
               'label_name must be provided')
            else:
